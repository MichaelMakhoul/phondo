...
- **reschedule_appointment — REVISED at implementation time:** no bespoke Cliniko reschedule code. The existing `handleRescheduleAppointment` already books the new leg through `handleBookAppointment` (which now dispatches to Cliniko) and frees the old leg through `cancelSingleAppointment` (which now propagates to Cliniko via `clinikoCancelExternal`). So a reschedule = new Cliniko appointment + cancellation of the old one, reusing the battle-tested verification, orphan-paging (SCRUM-377), and `rescheduled_from_id` chain logic verbatim. The practice sees a cancel + new booking rather than an in-place move; a single-`PUT` in-place move is a possible future optimization (follow-up), and the spec's earlier `updateAppointmentTime` client method remains for it.
