...
8. **Silent-Failure Hunter** — Launch `silent-failure-hunter` agent on all changed files. Check for:
...
9. **Security Review** — Invoke the `/security-review` skill on the pending changes. This is a broader security audit than the silent-failure-hunter and complements it. Covers:
   - Authentication / authorization holes (missing auth middleware, broken RBAC, bypassable role checks)
   - Secrets handling (hardcoded credentials, unsafe env var usage, secrets in logs)
   - Injection surfaces beyond SQL/XSS (SSRF, prompt injection, command injection, path traversal)
   - Dependency vulnerabilities introduced by the diff
   - OWASP top 10 coverage relevant to web + voice surfaces
   - Run this in parallel with #7 and #8 — they cover different blast radii.
10. **Code Simplifier** — Launch `code-simplifier` agent to ensure code is clean and minimal.
11. **Type Design** — If new TypeScript types/interfaces were introduced, launch `type-design-analyzer` to review type quality.

### Phase 3.5 — Client Simulation (runs after code review, before verification)

12. **Client Simulator** — Evaluate the change from the perspective of real end-users across multiple industry personas. For each relevant persona below, independently assess: "Would this make sense to me? Would I find this intuitive? Does this solve my actual problem?"
...
13. **QA Engineer** — After all reviews pass:
...
14. **Test Coverage** — If new logic was added without tests, flag the gap and suggest what tests should be written (ask user before writing them).
...
15. **Jira Ticket Creation** — For ANY issue, bug, UX gap, or improvement discovered during the review pipeline (phases 3, 3.5, or 4) that is **not fixed in the current task**:
