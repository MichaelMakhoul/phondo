      ...
      switch (msg.event) {
        ...
        case "start": {
          ...
          if (VOICE_PIPELINE === "gemini-live" && process.env.GEMINI_API_KEY) {
            ...
            geminiSystemPrompt += `\n- CONFIRM BOOKING DETAILS: AFTER the book_appointment TOOL CALL has returned a result AND you have copied the confirmation code from the TOOL RESPONSE (never invented one), read back ALL details to the caller: name, date, time, practitioner, and the tool-provided confirmation code. Then ALSO tell the caller: "You'll receive a confirmation text at the number you're calling from shortly — please check it and let us know if anything looks wrong." (This is important: the SMS gives the caller a chance to catch any mistakes before the appointment.) Then ask "Is everything correct?" If the caller says something is wrong, fix it (cancel and rebook with the correct details). Do NOT end the booking conversation without confirmation.`;
